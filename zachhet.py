...
class SolarEclipseApp(QWidget):
    def __init__(self):
        ...
        self.distance_from_earth = 200  # Начальное расстояние между Луной и Землей
    ...
    def initUI(self):
        ...
        self.distance_slider.setRange(100, 400)  # Установите диапазон по вашему усмотрению
    ...
    def update_moon_position(self):
        ...
        if self.moon_position > self.width():
            self.moon_position = -self.moon_radius  # Сбросить позицию Луны
        self.update()
    ...
    def paintEvent(self, event):
        ...
        # Рисуем Луну
        moon_x = self.moon_position + self.distance_from_earth  # Учитываем расстояние
        ...
        if (moon_x + self.moon_radius > sun_x) and (moon_x < sun_x + sun_radius):
            # Рисуем затмение
            painter.setBrush(QColor(0, 0, 0))  # Черный цвет для затмения
            painter.drawEllipse(moon_x, moon_y, self.moon_radius, self.moon_radius)
